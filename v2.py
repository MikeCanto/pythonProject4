...
# Función para obtener el hash SHA-256 de un mensaje
def obtener_hash(mensaje):
    """
    Esta función calcula el hash SHA-256 de un mensaje.

    Parámetros:
        mensaje (str): El mensaje a hashear.

    Retorno:
        str: El hash SHA-256 del mensaje.
    """
    hash_sha256 = hashlib.sha256()
    hash_sha256.update(mensaje.encode("utf-8"))
    return hash_sha256.hexdigest()

# Ejemplo de uso de la función obtener_hash
mensaje = "UYUYUYUYUY"
hash_mensaje = obtener_hash(mensaje)
print("Hash del mensaje:", hash_mensaje)

# Firma digital:

# 1. Alice genera un HASH h(M) del mensaje
hash_mensaje = obtener_hash(mensaje)
